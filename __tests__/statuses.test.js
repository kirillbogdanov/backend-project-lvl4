...
describe('requests', () => {
  ...
  beforeAll(async () => {
    app = createApp();
    session = await authenticateAsNewUser(app);
  });
  ...
  test('CRUD main flow', async () => {
    ...
    const readRes = await getEntity(app, prefix, status.id, session);
  ...
  describe('GET /statuses/:id/edit', () => {
    test('is unavailable for unauthenticated user', async () => {
      ...
      const res = await getEntity(app, prefix, status.id);

      expect(res.statusCode).toBe(302);
    });

    test('returns 404 if status with specified id does not exists', async () => {
      const res = await getEntity(app, prefix, 9999999, session);
  ...
  describe('DELETE /statuses/:id', () => {
    test('is unavailable for unauthenticated user', async () => {
      ...
      const patchedStatus = await app.objection.models.status.query()
        .findById(status.id);
      expect(patchedStatus.name).not.toBeUndefined();
    });
